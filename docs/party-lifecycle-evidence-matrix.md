...
| --- | --- | --- |
| Reviewed 4.9 LIVE private log analysis | [`game-log-pattern-analysis-2026-08-09.md`](game-log-pattern-analysis-2026-08-09.md#settled-requirement-live-party-status) | Supports creation, launch, and a named member connection only. |
...
| Unavailable lifecycle annotation | `party/party-lifecycle-transitions.unavailable` | Records missing invite, accept/join, disconnect, reconnect, leave, kick/removal, leader change, disband, and mid-party startup evidence. |
...
| Local voluntary leave | `PartyLeft` | Defer | None | Not captured in the reviewed build | Do not clear party state from absence of later records. Apply stale/session-boundary policy. |
...
| `not_in_party` | Direct evidence supports no current party. | Future supported terminal evidence only. | Supported creation or join evidence. |
...
- Missing terminal evidence is not terminal evidence. Absence of leave, disband, or disconnect lines must not clear party state immediately.
