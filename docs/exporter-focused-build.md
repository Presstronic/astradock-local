...
The Exporter configuration selects Blueprint Data, a validated environment, and JSON, CSV, or XML output. Test Export runs the same validation, scanning, profile, normalization, and progress pipeline without opening a save dialog or writing a file. JSON retains its array-of-records contract. CSV uses the deterministic `name,type,shared` columns, writes UTF-8 text with CRLF row endings, quotes every cell, doubles embedded quotes, and represents unknown `shared` values as an empty cell. XML uses a `<blueprints>` root with required `<name>`, `<type>`, and `<shared>` elements; unknown `shared` values use `xsi:nil="true"` rather than an invented boolean. XML output is shape-validating and deterministic but is not yet an authoritative Station XML contract.

The native export workflow is single-flight and reports measured file, record, and duplicate counts as scanning proceeds; when a percentage cannot be derived it remains explicitly indeterminate. Save selection is performed by Electron's native dialog with an appropriate extension filter and overwrite confirmation. Output is validated against the local provisional `name,type,shared` contract, then written as UTF-8 through a unique, permission-restricted temporary file, flushed before an atomic rename, and removed if cancellation or failure occurs before commit. Recoverable dialog and write failures receive one automatic retry. A successful export reveals the destination in the user's file manager; the renderer receives only the output filename and never the private output path. Cancellation is honored through scanning and before the final rename, while a commit already completed is never falsely reported as cancelled.

Station compatibility is not authoritative yet: no Station API or validator is configured in this repository. The local contract requires exactly `name`, `type`, and `shared` in stable name order, with `type` as a string and `shared` as `true`, `false`, or `null`; it rejects duplicate names, raw evidence fields, and malformed values before writing. This validates shape and privacy boundaries, not semantic completeness of unavailable fields.
