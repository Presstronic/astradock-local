...
| Compact layout | Between `1024x640` and `1279x719` CSS pixels, the app must preserve header health/source context, the promoted shard/region, PU replication connection, PU duration, and application duration values, current-state access, stream access, drilldown access, and global actions. The existing header information strip may wrap or occupy additional rows without changing field treatment, reading order, or keyboard order. Lower-priority columns/panels may collapse, stack, or move behind explicit controls. |
...
| Header/source health and primary telemetry | One consistently styled header information strip: Source health, Last activity, Parser, Source, shard+region, PU replication connection, PU duration, and application duration. Added fields match the existing metric box, label/value hierarchy, density, alignment, typography roles, borders/background, and interaction states. | Source/retry/detail and actionable telemetry items are buttons with visible focus and deterministic result focus; DOM and keyboard order follow the displayed field order | Meaningful health and connection transitions announce politely; duration ticks and ordinary freshness ticks do not spam | Health states include discovering, awaiting source, starting, live, paused, stale, recovering, disconnected/source missing, unsupported profile, degraded, stopped, error; telemetry preserves Current/Unknown/Transitioning/Stale/Disconnected/Unsupported as text, never color alone |
...
| Instruments | Header: shard+region, PU replication connection, PU duration, app duration. Rail: latest jurisdiction, monitored space, armistice, active critical warnings and other approved current-state facts. Header values are not duplicated in the rail. | Instruments are focusable only when actionable; selection filters/focuses stream or opens shared detail | Each instrument exposes value, state, freshness, provenance/confidence where material; duration ticks are not live announcements | Current/Last confirmed/Unknown/Transitioning/Stale/Disconnected/Unsupported are distinct text labels |
